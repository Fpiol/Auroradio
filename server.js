...
// 跨平台节奏缓存目录：
//  - Windows 保留原来的独立盘策略（默认 D 盘，禁止 C 盘）；
//  - macOS / Linux 使用应用数据目录或系统临时目录，避免 Windows 盘符假设导致缓存在 Mac 上直接失效。
const AURORADIO_DATA_DIR = process.env.AURORADIO_DATA_DIR || '';
const DEFAULT_BEAT_CACHE_DIR = IS_WINDOWS
  ? 'D:\\AuroradioCache\\beatmaps'
  : path.join(AURORADIO_DATA_DIR || path.join(os.tmpdir(), 'Auroradio'), 'beat-cache');
const BEATMAP_CACHE_DIR = process.env.AURORADIO_BEAT_CACHE_DIR
  || process.env.MINERADIO_BEAT_CACHE_DIR
  || DEFAULT_BEAT_CACHE_DIR;
// 出于安全考虑，Auroradio 不再让远端补丁在运行时改写本地应用文件（原 Mineradio 的快速补丁自改写机制）。
// 更新仍然走「检测 + 下载完整安装包」路径。
const RUNTIME_PATCH_APPLY_DISABLED = process.env.AURORADIO_ALLOW_RUNTIME_PATCH !== '1';
const APP_PACKAGE = readPackageInfo();
const APP_DISPLAY_NAME = APP_PACKAGE.productName || 'Auroradio';
const APP_ARTIFACT_BASENAME = (APP_DISPLAY_NAME || 'Auroradio').replace(/\s+/g, '');
...
function updatePlatform() {
  const value = String(process.env.MINERADIO_UPDATE_PLATFORM || process.platform || '').toLowerCase();
  if (value === 'mac' || value === 'macos') return 'darwin';
  if (value === 'windows') return 'win32';
  return value;
}
function updatePlatformAssetExtensions() {
  const platform = updatePlatform();
  if (platform === 'darwin') return ['dmg', 'pkg', 'zip'];
  if (platform === 'win32') return ['exe', 'msi', 'zip', '7z'];
  if (platform === 'linux') return ['appimage', 'deb', 'rpm', 'tar.gz', 'zip'];
  return ['dmg', 'pkg', 'exe', 'msi', 'zip', '7z'];
}
function updatePlatformAssetRegex(exts = updatePlatformAssetExtensions()) {
  const escaped = exts.map(ext => ext.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  return new RegExp('\\.(' + escaped.join('|') + ')$', 'i');
}
function defaultUpdateAssetName(version) {
  const v = version || APP_VERSION;
  const platform = updatePlatform();
  if (platform === 'darwin') return `${APP_ARTIFACT_BASENAME}-${v}-${process.arch === 'x64' ? 'x64' : 'arm64'}.dmg`;
  if (platform === 'linux') return `${APP_ARTIFACT_BASENAME}-${v}.AppImage`;
  return `${APP_ARTIFACT_BASENAME}-${v}-Setup.exe`;
}
...
function patchFileNameFromUpdateName(name, version) {
  const base = safeUpdateFileName(name || defaultUpdateAssetName(version), version);
  const knownExt = updatePlatformAssetExtensions()
    .sort((a, b) => b.length - a.length)
    .find(ext => base.toLowerCase().endsWith('.' + ext.toLowerCase()));
  return knownExt ? base.slice(0, -knownExt.length - 1) + '.patch.json' : base + '.patch.json';
}
function pickReleaseAsset(assets) {
  const list = Array.isArray(assets) ? assets : [];
  const platformAsset = updatePlatformAssetRegex();
  const preferred = list.find(a => platformAsset.test(a && a.name || ''))
    || list.find(a => /\.(dmg|pkg|exe|msi|zip|7z|appimage|deb|rpm|tar\.gz)$/i.test(a && a.name || ''))
    || list[0];
...
function normalizeManifestUpdateInfo(data) {
  ...
  return {
    ...
    release: {
      ...
      downloadUrl,
      asset: assetInfo,
      patch: RUNTIME_PATCH_APPLY_DISABLED ? null : patchInfo,
      patchAvailable: !RUNTIME_PATCH_APPLY_DISABLED && !!(patchInfo && patchInfo.downloadUrl && compareVersions(latestVersion, APP_VERSION) > 0),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `${APP_ARTIFACT_BASENAME}/${APP_VERSION}` },
    });
...
function beatCacheRootInfo() {
  ...
  if (!IS_WINDOWS) {
    // 非 Windows：使用应用数据/临时目录，始终允许并视为可用（目录会按需创建）。
...
function safePathStat(target) {
  try {
    if (!target || !fs.existsSync(target)) return { exists: false, bytes: 0, files: 0 };
    const stat = fs.statSync(target);
    if (stat.isFile()) return { exists: true, bytes: stat.size, files: 1 };
    if (!stat.isDirectory()) return { exists: true, bytes: stat.size || 0, files: 0 };
    let bytes = 0;
    let files = 0;
    const stack = [target];
    while (stack.length) {
    ...
    return { exists: false, bytes: 0, files: 0, error: e.message };
  }
}
function privacyStorageSummary() {
  const beat = beatCacheRootInfo();
  const items = [
    { key: 'neteaseCookie', label: '网易云登录 Cookie', path: COOKIE_FILE, removable: true },
    { key: 'qqCookie', label: 'QQ 音乐登录 Cookie', path: QQ_COOKIE_FILE, removable: true },
    { key: 'beatCache', label: '节奏分析缓存', path: beat.dir, removable: true },
    { key: 'updateDownloads', label: '更新安装包缓存', path: UPDATE_DOWNLOAD_DIR, removable: true },
  ].map(item => ({ ...item, ...safePathStat(item.path) }));
  return {
    ok: true,
    productName: APP_DISPLAY_NAME,
    platform: process.platform,
    appDataDir: AURORADIO_DATA_DIR || '',
    beatCache: {
      enabled: beat.allowed && beat.available,
      dir: beat.dir,
      mode: beat.allowed && beat.available ? 'disk' : 'memory-only',
      reason: !beat.allowed ? 'C_DRIVE_DISABLED' : (!beat.available ? 'TARGET_DRIVE_UNAVAILABLE' : ''),
    },
    items,
    browserLocalStorage: {
      note: '自定义封面、歌词布局、视觉存档、部分本地节奏映射保存在当前浏览器 localStorage，可在应用内隐私中心一键清理。',
      keys: [
        'mineradio-custom-covers',
        'mineradio-custom-lyrics',
...
function deleteFileOrDirectory(target) {
  ...
  const allowedRoots = [
    path.resolve(AURORADIO_DATA_DIR || os.tmpdir()),
    path.resolve(UPDATE_WORK_DIR),
    path.resolve(BEATMAP_CACHE_DIR),
    path.resolve(os.tmpdir()),
  ];
  const safe = allowedRoots.some(root => resolved === root || resolved.startsWith(root + path.sep))
    || resolved === path.resolve(COOKIE_FILE)
    || resolved === path.resolve(QQ_COOKIE_FILE);
  if (!safe) throw new Error('UNSAFE_PRIVACY_DELETE_TARGET');
  if (!fs.existsSync(resolved)) return false;
  fs.rmSync(resolved, { recursive: true, force: true });
  return true;
}
function clearPrivacyTargets(targets) {
  const requested = new Set(Array.isArray(targets) && targets.length ? targets : ['cookies', 'beatCache', 'updateDownloads']);
  const cleared = [];
  const failed = [];
  const actions = [
    { group: 'cookies', key: 'neteaseCookie', path: COOKIE_FILE },
    { group: 'cookies', key: 'qqCookie', path: QQ_COOKIE_FILE },
    { group: 'beatCache', key: 'beatCache', path: beatCacheRootInfo().dir },
    { group: 'updateDownloads', key: 'updateDownloads', path: UPDATE_DOWNLOAD_DIR },
  ];
  actions.forEach(action => {
    if (!requested.has(action.group) && !requested.has(action.key)) return;
    try {
      const removed = deleteFileOrDirectory(action.path);
      cleared.push({ key: action.key, removed, path: action.path });
    } catch (e) {
      failed.push({ key: action.key, error: e.message || 'DELETE_FAILED' });
    }
  });
  return { ok: failed.length === 0, cleared, failed, summary: privacyStorageSummary() };
}
...
async function fetchLatestYmlUpdateInfo(reason) {
  ...
  const latestYmlUrl = `https://github.com/${encodeURIComponent(UPDATE_CONFIG.owner)}/${encodeURIComponent(UPDATE_CONFIG.repo)}/releases/latest/download/${latestYmlFileName()}`;
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `${APP_ARTIFACT_BASENAME}/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        asset,
        patch: RUNTIME_PATCH_APPLY_DISABLED ? null : patch,
        patchAvailable: !RUNTIME_PATCH_APPLY_DISABLED && !!(patch && patch.downloadUrl && compareVersions(latestVersion, APP_VERSION) > 0),
...
function startUpdatePatchJob(info) {
  if (RUNTIME_PATCH_APPLY_DISABLED) {
    return {
      ok: false,
      error: 'RUNTIME_PATCH_DISABLED',
      message: 'Auroradio 已禁用运行时快速补丁，请下载完整安装包更新。',
    };
  }
  const release = info && info.release ? info.release : {};
  ...
  const job = {
    ...
    total: patch.size || 0,
    mode: 'patch',
    fileName: patch.name || patchFileNameFromUpdateName('', version),
    filePath: '',
    version,
    downloadUrl,
    downloadCandidates,
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/privacy/summary') {
    sendJSON(res, privacyStorageSummary());
    return;
  }

  if (pn === '/api/privacy/clear') {
    if (req.method !== 'POST') {
      sendJSON(res, { ok: false, error: 'METHOD_NOT_ALLOWED' }, 405);
      return;
    }
    try {
      const body = await readRequestBody(req);
      sendJSON(res, clearPrivacyTargets(body && body.targets));
    } catch (err) {
      sendJSON(res, { ok: false, error: err.message || 'PRIVACY_CLEAR_FAILED' }, 500);
    }
    return;
  }
