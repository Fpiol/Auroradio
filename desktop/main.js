...
const APP_NAME = 'Auroradio';
const APP_USER_MODEL_ID = 'com.auroradio.desktop';
...
function getAppIconPath() {
  const candidates = process.platform === 'win32'
    ...
    : (process.platform === 'darwin' ? [APP_ICON_ICNS, APP_ICON_PNG] : [APP_ICON_PNG, APP_ICON_ICO]);
  return candidates.find((candidate) => candidate && fs.existsSync(candidate)) || undefined;
}

function configureMacApplication() {
  if (process.platform !== 'darwin') return;
  try {
    if (app.dock && fs.existsSync(APP_ICON_PNG)) app.dock.setIcon(APP_ICON_PNG);
  } catch (e) {
    console.warn('Mac dock icon setup skipped:', e.message);
  }
  try {
    Menu.setApplicationMenu(Menu.buildFromTemplate([
      ...
      },
      {
        label: '播放',
        submenu: [
          { label: '播放 / 暂停', accelerator: 'Command+Option+Space', click: () => sendGlobalHotkeyAction('togglePlay') },
          { label: '上一首', accelerator: 'Command+Option+Left', click: () => sendGlobalHotkeyAction('prevTrack') },
          { label: '下一首', accelerator: 'Command+Option+Right', click: () => sendGlobalHotkeyAction('nextTrack') },
          { type: 'separator' },
          { label: '桌面歌词', accelerator: 'Command+Option+L', click: () => sendGlobalHotkeyAction('toggleDesktopLyrics') },
        ],
      },
      {
        label: '编辑',
    ...
    console.warn('Mac application menu setup skipped:', e.message);
  }
}

function configureMacMediaKeys() {
  if (process.platform !== 'darwin') return;
  const bindings = [
  ...
  bindings.forEach(([accelerator, action]) => {
    try {
      globalShortcut.register(accelerator, () => sendGlobalHotkeyAction(action));
    } catch (e) {
      console.warn(`Mac media key registration skipped: ${accelerator}`, e.message);
    }
  });
}
...
function configureWallpaperWindowForPlatform(win) {
  if (!win || win.isDestroyed()) return;
  ...
  if (process.platform === 'darwin') {
    try {
      ...
      console.warn('Mac wallpaper workspace setup skipped:', e.message);
    }
    try {
      win.setAlwaysOnTop(false);
    } catch (_) {}
    // macOS 不开放 Windows WorkerW 这类“嵌入桌面图标层后方”的能力。
    // 这里保留同一套视觉/控制/性能逻辑，把它作为跨空间、不可点击的沉浸背景窗显示。
    console.warn('Mineradio wallpaper is running in macOS ambient-window mode.');
  }
}
...
function createWallpaperWindow(payload = {}) {
  ...
  wallpaperWindow.once('ready-to-show', () => {
    ...
    configureWallpaperWindowForPlatform(wallpaperWindow);
    sendWallpaperState();
  });
...
ipcMain.handle('auroradio-show-path-in-folder', async (_event, targetPath) => {
  try {
    ...
    return { ok: false, error: e.message || 'SHOW_PATH_FAILED' };
  }
});
...
async function createWindow() {
  ...
  process.env.AURORADIO_DATA_DIR = app.getPath('userData');
  ...
  if (isMac) {
    // macOS 原生外壳：隐藏标题栏但保留左上角交通灯，系统负责圆角、阴影与毛玻璃。
    Object.assign(mainWindowOptions, {
    ...
    // Windows/Linux 继续使用自绘无边框透明窗，由渲染层画圆角。
    Object.assign(mainWindowOptions, {
      frame: false,
      transparent: true,
      backgroundColor: '#00000000',
